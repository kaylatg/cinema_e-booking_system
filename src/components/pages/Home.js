...
function Home() {
    ...
    React.useEffect(() => {
      setShowError(
        !moviesUpcoming.some(filterMovies) && !moviesPlaying.some(filterMovies)
      );
    }, [moviesUpcoming, moviesPlaying, searchQuery, searchType]);
        ...
        <home className="home">
          ...
          <div className="searchContainer">
            <select className="searchType" value={searchType} onChange={handleSearchTypeChange}>
              <option value="title" selected={searchType === 'title'}>
                Title
              </option>
              <option value="genre" selected={searchType === 'genre'}>
                Genre
              </option>
            </select>
            <div>
          ...
          <div className="comingSoon-container">
            ...
            {moviesUpcoming.filter(filterMovies).length > 0 ? (
              ...
              <div className="no-movies">Sorry, there are no upcoming movies to show at the moment.</div>
            )}
          </div>
          ...
          <div className="currentlyShowing-container">
            ...
            {moviesPlaying.filter(filterMovies).length > 0 ? (
              ...
              <div className="no-movies">Sorry, there are no currently showing movies to display.</div>
            )}
          </div>
        </home>
      </>
